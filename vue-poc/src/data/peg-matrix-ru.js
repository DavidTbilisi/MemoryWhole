...
// Generate pegmatrix RU images by combining audio (tens digit) and visual (units digit) side by side
const PEG_MATRIX_RU_IMAGES = (() => {
  const out = {};
  for (let i = 0; i <= 99; i++) {
    const key = String(i).padStart(2, '0');
    ...
    out[key] = {
      audio: PEG_IMAGES_RU.audio[audioIdx],
      visual: PEG_IMAGES_RU.visual[visualIdx]
    };
  }
  return out;
})();

export { PEG_AUDIO_RU, PEG_VISUAL_RU, PEG_IMAGES_RU, PEG_MATRIX_RU_DATA, PEG_MATRIX_RU_IMAGES }
