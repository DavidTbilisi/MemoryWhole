...
function refreshHomeMastery() {
  ...
  decks.forEach(deck => {
    ...
    // Use persisted per-deck peak so home badges don't show regressions
    const masteryPeaks = JSON.parse(localStorage.getItem(MASTERY_PEAK_KEY) || '{}');
    const storedPeak = masteryPeaks[deck] || 0;
    const display = Math.max(mastery.score, storedPeak);
    const pct = display.toFixed(0);
    const color = display >= 80 ? '#10b981' : display >= 50 ? '#f59e0b' : '#ef4444';
    el.innerHTML = `
      <svg width="44" height="44" viewBox="0 0 44 44">
        ...
        <circle cx="22" cy="22" r="18" fill="none" stroke="${color}" stroke-width="4"
          stroke-dasharray="${(display / 100 * 113).toFixed(1)} 113"
